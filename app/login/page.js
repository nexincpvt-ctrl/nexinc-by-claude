...
export default function LoginPage() {
  ...
  const handleValidation = () => {
    const tempErrors = {};
    if (!email.trim()) {
      tempErrors.email = "Please type in your email address.";
    }
    if (!password) {
    ...
    try {
      ...
      if (error) {
        if (error.message.toLowerCase().includes("email not confirmed") || error.message.toLowerCase().includes("email not verified")) {
          // Immediately redirect unverified users to verify-otp page to complete verification
          router.push(`/verify-otp?email=${encodeURIComponent(email)}`);
        } else if (error.message.toLowerCase().includes("invalid login credentials")) {
          setErrors({ submit: "Hmm, that didn't match — want to try again?" });
        } else {
      ...
      setErrors({ submit: "Something went wrong on our end. Let's try again in a moment!" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setErrors({ submit: "Could not connect to Google. Please check your network!" });
      setLoading(false);
    }
  };
      ...
      <div className="bg-brand-card p-8 sm:p-10 rounded-3xl shadow-xl shadow-brand-dark/5 border border-brand-dark/5 w-full">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-brand-dark/60 text-sm mt-1">
            Let's get you signed in to NexInc.
          </p>
        </div>

        {errors.submit && (
          <div className="p-4 mb-6 bg-brand-error/10 border border-brand-error/25 text-brand-error text-sm rounded-2xl flex flex-col gap-3">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div>
            <div className="flex justify-between items-center mb-1 ml-1">
              <label className="block text-xs font-semibold uppercase tracking-wider text-brand-dark/60 select-none">
                Password
              </label>
              <Link
