 ...
 */
export async function createChatSession(supabase, userId, section, title = "New chat") {
  const { data, error } = await supabase
    .from("chat_sessions")
    .insert({
      user_id: userId,
...
export async function getMessages(supabase, sessionId) {
  ...
  // Parse potential multimodal or image JSON serialized messages
  return data.map((m) => {
    try {
      const parsed = JSON.parse(m.content);
      if (parsed && parsed.message_type === "image") {
        return {
          ...m,
          message_type: "image",
 ...
 * @param {string} [params.message_type] - Optional type identifier (e.g. 'image').
 * @param {string} [params.image_url] - Optional uploaded file url.
 * @returns {Promise<object>} The newly inserted message row.
 */
export async function addMessage(supabase, { sessionId, userId, role, content, modelUsed, message_type, image_url }) {
